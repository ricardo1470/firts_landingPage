...
// el área está representada en m2, harvestYear es el año en el que se sembró el cuartel
const paddocks = [
    {
        paddockManagerId: 6,
        farmId: 1,
...
function listPaddockManagersByName() {
    // CODE HERE
    return paddockManagers.sort((a, b) => a.name.localeCompare(b.name)).map((paddockManager) => paddockManager.name);
}
...
function sortPaddockTypeByTotalArea() {
    // CODE HERE
    return paddockType.sort((a, b) => b.area - a.area).map((paddockType) => paddockType.name);
}
...
function sortFarmManagerByAdminArea() {
    // CODE HERE
    return farms.sort((a, b) => b.area - a.area).map((farmManager) => farmManager.name);
}

// 4 Objeto en que las claves sean los nombres de los campos y los valores un arreglo con los ruts
// de sus administradores ordenados alfabéticamente por nombre.
function farmManagerNames() {
    // CODE HERE
    const farmManagerNames = {};
    farms.forEach((farm) => {
        farmManagerNames[farm.name] = paddockManagers.filter((paddockManager) => farm.id === paddockManager.farmId).map((paddockManager) => paddockManager.name).sort((a, b) => a.localeCompare(b));
    }
    );
...
function biggestAvocadoFarms() {
    // CODE HER
    return farms.filter((farm) => farm.area > 2).sort((a, b) => b.area - a.area).map((farm) => farm.name);
}
...
function biggestCherriesManagers() {
    // CODE HERE
    return farms.filter((farm) => farm.name === "FORESTAL Y AGRICOLA LO ENCINA").filter((farm) => farm.area > 1000).sort((a, b) => a.name.localeCompare(b.name)).map((farm) => farm.name);
}
...
function farmManagerPaddocks() {
    ...
    farms.forEach((farm) => {
        farmManagerPaddocks[farm.name] = farms.filter((farm) => farm.name === farm.name).map((farm) => farm.name).sort((a, b) => a.localeCompare(b));
    }
    );
...
function paddocksManagers() {
    // CODE HERE
    const paddocksManagers = {};
    paddocks.forEach((paddock) => {
        paddocksManagers[`${paddock.type}-${paddock.year}`] = paddockManagers.filter((paddockManager) => paddock.id === paddockManager.paddockId).map((paddockManager) => paddockManager.name);
    }
    );
...
function newManagerRanking() {
    // CODE HERE
    /*const newManager = {
        id: paddockManagers.length + 1,
        name: "Nuevo Administrador",
        farmId: farms.find((farm) => farm.name === "AGRICOLA SANTA ANA").id,
        paddockId: paddocks.find((paddock) => paddock.type === "NOGALES" && paddock.year === 2017).id
    };
